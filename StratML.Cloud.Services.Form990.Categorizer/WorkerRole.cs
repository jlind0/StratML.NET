        ...
        private async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            queue.RegisterMessageHandler(async (msg, token) =>
            {
                ...
                })
                {
                    ...
                    await sb.Queues.CreateOrUpdateAsync("stratml", "stratml", version + "-" + type, new SBQueue());
                    var q = new QueueClient(ConfigurationManager.AppSettings["connectionString"], version + "-" + type);
                    await q.SendAsync(new Message(Encoding.UTF8.GetBytes(url)));
                    await q.CloseAsync();
                }
            },
            new MessageHandlerOptions(evt => Task.FromException(evt.Exception))
            { MaxConcurrentCalls = 8, AutoComplete = true });
            while (true)
            {
